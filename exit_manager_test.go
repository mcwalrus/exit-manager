...
func TestRegisterPreShutdown(t *testing.T) {
	...
	t.Run("pre-shutdown hooks execute before HTTP server shutdown", func(t *testing.T) {
		em := testExitManager(t)
		...
		em.RegisterPreShutdown(func() {
			mu.Lock()
			events = append(events, "pre-shutdown")
			mu.Unlock()
		})

		err := em.RegisterServer(server.Config)
		if err != nil {
		...
		mu.Lock()
		defer mu.Unlock()
		if len(events) < 1 || events[0] != "pre-shutdown" {
			t.Errorf("expected pre-shutdown to execute first, got events: %v", events)
		}
	})
}
...
func TestRegisterServer(t *testing.T) {
	...
	t.Run("server shutdown with timeout", func(t *testing.T) {
		...
		em.SetServerTimeout(10 * time.Millisecond)
		...
		// Server should eventually be closed despite timeout
		if !serverIsClosed(server) {
			t.Error("server should be closed after shutdown")
		}
	})

	t.Run("multiple HTTP servers shutdown concurrently", func(t *testing.T) {
		em := testExitManager(t)

		// Create multiple servers
		servers := make([]*httptest.Server, 3)
		for i := range servers {
			servers[i] = httptest.NewServer(httpHandler)
		...
		for i, server := range servers {
			err := em.RegisterServer(server.Config)
			if err != nil {
				t.Fatalf("unexpected error registering server %d: %v", i, err)
			}
		}
		...
		// Verify all servers are shutdown
		for i, server := range servers {
			if !serverIsClosed(server) {
				t.Errorf("server %d was not shutdown", i)
			}
		}
	})
}

func TestSetServerTimeout(t *testing.T) {
	t.Parallel()
	...
	t.Run("zero timeout means no timeout", func(t *testing.T) {
		...
		// Set zero timeout (no timeout)
		em.SetServerTimeout(0)
